...
with tab1:
    ...
    layak_train  = len(df_train[df_train['Label'].str.strip() == 'Layak'])
    tidak_train  = len(df_train[df_train['Label'].str.strip() != 'Layak'])
    layak_test   = len(df_test[df_test['Label'].str.strip() == 'Layak'])
    tidak_test   = len(df_test[df_test['Label'].str.strip() != 'Layak'])
...
with tab2:
    ...
    with col1:
        layak = len(df_train[df_train['Label'].str.strip() == 'Layak'])
        st.markdown(f'<div class="metric-box metric-green"><h2>{layak}</h2><p>Layak</p></div>', unsafe_allow_html=True)
    with col2:
        tidak = len(df_train[df_train['Label'].str.strip() != 'Layak'])
...
with tab3:
    ...
    col1, col2 = st.columns(2)
    with col1:
        layak = len(df_test[df_test['Label'].str.strip() == 'Layak'])
        st.markdown(f'<div class="metric-box metric-green"><h2>{layak}</h2><p>Layak</p></div>', unsafe_allow_html=True)
    with col2:
        tidak = len(df_test[df_test['Label'].str.strip() != 'Layak'])
        st.markdown(f'<div class="metric-box metric-red"><h2>{tidak}</h2><p>Tidak Layak</p></div>', unsafe_allow_html=True)
